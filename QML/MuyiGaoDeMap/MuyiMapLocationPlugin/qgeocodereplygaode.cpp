...
JasonMonger kMonger;
QGeoCodeReplyGaoDe::QGeoCodeReplyGaoDe(QNetworkReply *reply, QObject *parent)
...
QGeoCodeReplyGaoDe::~QGeoCodeReplyGaoDe()
{
    if (m_reply)
        m_reply->deleteLater();
}

void QGeoCodeReplyGaoDe::abort()
{
    if (!m_reply)
        return;
...
void QGeoCodeReplyGaoDe::networkReplyFinished()
{
    if (!m_reply)
        return;
...
void QGeoCodeReplyGaoDe::networkReplyError(QNetworkReply::NetworkError error)
{
    Q_UNUSED(error)
    if (!m_reply)
        return;
